...
class Enemy {
  ...
  takeDamage(player,scene) {
    ...
    const str = weapon.str
    const cd = weapon.cd
    const stats = player.stats
    const damage = calculateDamage(dmg ,str + stats.added.str + stats.str,cd + stats.add.cd + stats.cd,player.levels.combat * 4 / 10 + weapon.bm, weapon.pm)
    console.log(damage)
...
class Weapon {
  constructor (name="Wood Sword", description="If you are reading this, I'm sorry...", dmg=1, str=0, cd=0, enchants=[], bm=0,pm=0) {
    this.name = name
...
class Skill {
  ...
  calcNextLevelExp() {
    this.nextLevelExp = this.nextLevelExp ** 1.5
  }
  calcTNL() {
...
class Armor {
  constructor(name,description,defense,health,str,cd,cc) {
    this.name = name
    this.description = description
    this.defense = defense
    this.health = health
...
class Player {
  ...
  updateProfileStats() {
    for (let x in this.equipped) {
      if (this.equipped[x].str) this.profileStats.str += this.equipped[x].str
      if (this.equipped[x].cd) this.profileStats.cd += this.equipped[x].cd
      if (this.equipped[x].cc) this.profileStats.cc += this.equipped[x].cc
      if (this.equipped[x].isArmor) {
        this.defense += this.equipped[x].defense
        this.maxHealth += this.equipped[x].health
      }
    }
    this.profileStats.str += this.baseStats.str
    this.profileStats.cd += this.baseStats.cd
    this.baseStats.cc += this.baseStats.cc
  }
}
...
const gs = new Weapon("Giant's Sword","A sword once wielded by only by giants and now, you.",550,60,40,[ofa,sharpness])
const player = new Player(100,100)
//zombie.takeDamage(calculateDamage(550 * 5,543 + (60 * 5),426 + (40* 5),19.2,0))
console.log(calculateDamage(550 * 5,275 + (60 * 5),298 + (40* 5),14.4,0))
